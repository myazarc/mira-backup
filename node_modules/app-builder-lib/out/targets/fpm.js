...
class FpmTarget extends _core().Target {
  ...
  build(appOutDir, arch) {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      ...
      const args = ["-s", "dir", "-t", target, "--architecture", target === "pacman" && arch === _builderUtil().Arch.ia32 ? "i686" : (0, _builderUtil().toLinuxArchString)(arch), "--name", appInfo.linuxPackageName, "--force", "--after-install", scripts[0], "--after-remove", scripts[1], "--description", (0, _appInfo().smarten)(target === "rpm" ? _this3.helper.getDescription(options) : `${synopsis || ""}\n ${_this3.helper.getDescription(options)}`), "--version", appInfo.version, "--package", artifactPath];
      ...
      if (target !== "rpm" && (yield (0, _macosVersion().isMacOsSierra)())) {
...
function writeConfigFile(_x, _x2, _x3) {
  return _writeConfigFile.apply(this, arguments);
} function _writeConfigFile() {
  _writeConfigFile = (0, _bluebirdLst().coroutine)(function* (tmpDir, templatePath, options) {
